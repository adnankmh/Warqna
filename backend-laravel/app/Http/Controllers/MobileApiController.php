...
class MobileApiController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        ...
        if (isset($data['pasha_style'])) {
            $profile->pasha_style = $data['pasha_style'] === 'black' ? 'red' : $data['pasha_style'];
        }
        $profile->save();
    ...
    public function activateStoreInventoryV183(Request $request)
    {
        ...
        $activatableCategories=[
            'table','card_back','name_color','text_color','badge','pasha_style',
            'name_frame','effect','emoji_pack','profile_cover','xp_booster',
        ];
        return DB::transaction(function() use($user,$data,$activatableCategories){
            // Lock the owned row before changing the active item. Concurrent
            // taps are serialized so a category cannot end with two actives.
            $inventory=$user->inventoryItems()
                ->whereHas('storeItem',fn($q)=>$q->where('key',$data['key'])->whereIn('category',$activatableCategories)->where('active',true))
                ->with('storeItem')->latest('id')->lockForUpdate()->first();
            if(!$inventory) return response()->json(['ok'=>false,'message'=>'العنصر غير موجود في مقتنياتك أو لم يعد متاحاً.'],404);
            if($inventory->expires_at && $inventory->expires_at->isPast()) {
                $inventory->delete();
                return response()->json(['ok'=>false,'message'=>'انتهت صلاحية العنصر قبل التفعيل.'],410);
            }

            $item=$inventory->storeItem;
            $payload=$item->payload ?: [];
            $isBooster=$item->category==='xp_booster';
            $hours=$isBooster ? max(1,(int)($payload['activate_hours'] ?? 24)) : null;
            $user->inventoryItems()
                ->where('id','!=',$inventory->id)
                ->whereHas('storeItem',fn($q)=>$q->where('category',$item->category))
                ->update(['active'=>false]);

            $inventory->update([
                'active'=>true,
                'activated_at'=>now(),
                'expires_at'=>$isBooster ? now()->addHours($hours) : $inventory->expires_at,
            ]);
            ...
            return response()->json([
                'ok'=>true,
                'message'=>$isBooster ? 'تم تفعيل المسرّع لمدة '.$hours.' ساعة.' : 'تم تفعيل العنصر المملوك واعتماده على الحساب.',
                'category'=>$item->category,
                'payload'=>$payload,
                'profile'=>$user->profile?->fresh(),
    ...
    private function activateStoreItem(User $user, StoreItem $item): void
    {
        ...
        switch ($item->category) {
            ...
            case 'pasha_style':
                ...
                $profile->pasha_style=$style==='black' ? 'red' : $style;
