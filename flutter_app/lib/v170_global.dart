    ...
    return PremiumPanel(
      child: Padding(
        ...
        child: Column(children: [
          ...
          const SizedBox(height:8),
          if (isPasha)
            SizedBox(width:double.infinity,child:FilledButton.icon(onPressed:() async { if(owned && !product.reusable){ final ok=await controller.activateProduct(product); if(context.mounted)showToast(context,ok?'تم تفعيل ${controller.nameFor(product)}.':(controller.lastStoreError ?? 'تعذر التفعيل.')); return; } final confirmed=await showDialog<bool>(context:context,builder:(dialogContext)=>AlertDialog(icon:Image.asset('assets/images/pasha.png',height:100,fit:BoxFit.contain),title:Text(controller.nameFor(product)),content:Text('${controller.descriptionFor(product)}\n\nالسعر: ${formatNumber(controller.priceFor(product))} توكن'),actions:[TextButton(onPressed:()=>Navigator.pop(dialogContext,false),child:const Text('إلغاء')),FilledButton(onPressed:()=>Navigator.pop(dialogContext,true),child:Text(owned?'تفعيل':'شراء'))])); if(confirmed==true && context.mounted){ if(owned){final ok=await controller.activateProduct(product);if(context.mounted)showToast(context,ok?'تم التفعيل.':(controller.lastStoreError ?? 'تعذر التفعيل.'));}else{final ok=await controller.buy(product);if(context.mounted)showToast(context,ok?'تم شراء الباشا وتفعيله.':(controller.lastStoreError ?? 'تعذر الشراء. تحقق من الاتصال والرصيد.'));}} },icon:const Icon(Icons.workspace_premium),label:Text(owned?'تفعيل الباشا':'شراء الباشا'),style:FilledButton.styleFrom(minimumSize:const Size.fromHeight(50))))
          else
            Row(children:[Expanded(child:OutlinedButton.icon(onPressed:()=>showProductPreview(context,controller,product),icon:const Icon(Icons.visibility_outlined,size:17),label:Text(L.t(controller.localeCode,'preview'),maxLines:1,overflow:TextOverflow.ellipsis),style:OutlinedButton.styleFrom(minimumSize:const Size.fromHeight(48),padding:const EdgeInsets.symmetric(horizontal:8)))),const SizedBox(width:7),Expanded(child:FilledButton(onPressed:() async { if(owned && !product.reusable){final ok=await controller.activateProduct(product);if(context.mounted)showToast(context,ok?'تم تفعيل ${controller.nameFor(product)}.':(controller.lastStoreError ?? 'تعذر التفعيل.'));return;}await showProductPreview(context,controller,product);},style:FilledButton.styleFrom(minimumSize:const Size.fromHeight(48),padding:const EdgeInsets.symmetric(horizontal:8)),child:FittedBox(child:Text(owned&&!product.reusable?'تفعيل':L.t(controller.localeCode,'buy'),style:const TextStyle(fontWeight:FontWeight.w900)))))]),
        ]),
      ),
    );
  }
