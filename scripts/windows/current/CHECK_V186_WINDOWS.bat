...
python tools\test_v184_flutter_quality_contract.py || goto :fail
python tools\test_v184_official_game_rules_contract.py || goto :fail
python tools\test_v185_world_class_contract.py || goto :fail
python tools\test_v186_engine_integrity_contract.py || goto :fail
...
php backend-laravel\tools\test-v186-engine-integrity.php || goto :fail
...
echo Warqnaa V0.3.5 v186 source package passed local preflight.
exit /b 0
:fail
echo Warqnaa V0.3.5 v186 preflight failed.
exit /b 1
